...
const int throttle_low = 1100;//ESCの最小のPWM幅（800us以上じゃないと認識しない）
Servo throttle6;
Servo throttle10;
Servo throttle11;
Servo throttle9;
...
void setup() {
  ...
  //モーターのセットアップ
  throttle6.attach(6);
  throttle10.attach(10);//arduinoの9番ピンに接続
  throttle11.attach(11);//arduinoの9番ピンに接続
  throttle9.attach(9);

  level = throttle_low;//スロットルを最小に設定
  throttle6.writeMicroseconds(level);//PWMを初期化
  throttle10.writeMicroseconds(level);//PWMを初期化
  throttle11.writeMicroseconds(level);//PWMを初期化
  throttle9.writeMicroseconds(level);//PWMを初期化
...
// ================================================================
/*微分器(ローパスフィルタ付き)
double Differential_lowpass(double input, double frecentry, double lowpass_frecentry){
...
void loop() {
  //制御系
            static int frag = 0;
            if(frag == 0){
              for(int i = level; i <= 1300; i+= 10){
                //Serial.println(i);
                throttle6.writeMicroseconds(i);
                throttle11.writeMicroseconds(i);
                throttle10.writeMicroseconds(i);
                throttle9.writeMicroseconds(i);
                delay(50);
            ...
            Serial.println("\t"); 
            double rpm = 0, kp = 1, ki = -1.5, kd =-0.1, I, D,target = -15;
